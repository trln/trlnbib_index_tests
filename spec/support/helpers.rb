...
module Helpers
  # send a GET request to the default Solr request handler
  # with the indicated query
  # @param query [String] the value of the 'q' param to be sent to Solr
  # @return [RSpecSolr::SolrResponseHash] object for rspec-solr testing
  #  the Solr response, with no facets, and only the id field for each Solr doc
  ...
  # @param query [String] the value of the 'q' param to be sent to Solr
  # @return [RSpecSolr::SolrResponseHash] object for rspec-solr testing
  #  the Solr response, with no facets, and only the id and
  #  title field for each Solr doc
  ...
  # send a GET request to the default Solr request handler
  # with the indicated Solr parameters
  # @param solr_params [Hash] the key/value pairs to be sent
  #  to Solr as HTTP parameters, in addition to
  ...
  # @param solr_params [Hash] the key/value pairs to be sent
  #  to Solr as HTTP parameters, in addition to
  #  those to get only id fields and title
  #  and no facets in the response
  # @return [RSpecSolr::SolrResponseHash] object for rspec-solr testing
  #  the Solr response, with no facets, and only the id and
  #  short title field for each Solr doc
  ...
  #####################################
  # AUTHOR SEARCH HELPERS
  #####################################

  def author_search_duke(query_str)
    local_scope_duke.merge(author_search_args(query_str))
  end

  def author_search_nccu(query_str)
    local_scope_nccu.merge(author_search_args(query_str))
  end

  def author_search_ncsu(query_str)
    local_scope_ncsu.merge(author_search_args(query_str))
  end

  def author_search_unc(query_str)
    local_scope_unc.merge(author_search_args(query_str))
  end
  ...
  def author_bf
    ...
    'linear(map(publication_year_isort,'\
    "#{current_year_plus_two},10000,"\
    "#{current_year_minus_ten},"\
    'abs(publication_year_isort)),11,0)^5'
  end

  #####################################
  # SUBJECT SEARCH HELPERS
  #####################################
  ...
  def subject_bq(query_str)
    "title_main_indexed_t:(#{query_str})^500, "\
  ...
  # send a GET request to the indicated Solr request handler
  # with the indicated Solr parameters
  # @param solr_params [Hash] the key/value pairs
  # to be sent to Solr as HTTP parameters
  # @param req_handler [String] the pathname of the desired
  # Solr request handler (defaults to 'select')
  # @return [RSpecSolr::SolrResponseHash] object for
  # rspec-solr testing the Solr response
  ...
  # response documents will only have id fields,
  # and there will be no facets in the response
  # @return [Hash] Solr HTTP params to reduce the size of the Solr responses
  def doc_ids_only
    { 'fl' => 'id', 'facet' => 'false' }
  end

  # response documents will only have id and title_245a_display fields,
  # and there will be no facets in the response
  # @return [Hash] Solr HTTP params to reduce the size of the Solr responses
  def doc_ids_titles
  ...
  def solr_schema
    @solr_schema ||= solr_conn.send_and_receive(
      'admin/file/', method: :get,
                     params: { 'file' => 'schema.xml', wt: 'xml' }
    )
  end

  def solr_config_xml
    @solrconfig_xml = solr_conn.send_and_receive(
      'admin/file/', method: :get,
                     params: { 'file' => 'solrconfig.xml', wt: 'xml' }
    )
  end
end
